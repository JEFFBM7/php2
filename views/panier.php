                ...
                <div class="lg:w-2/3">
                    <div class="bg-white dark:bg-gray-800 rounded-xl shadow-md overflow-hidden">
                        <div class="p-6 border-b border-gray-200 dark:border-gray-700 flex justify-between items-center">
                            <h2 id="cart-title-heading" class="text-xl font-bold text-gray-900 dark:text-white flex items-center">
                                ...
                                <span id="cart-title-text">Votre panier (<?= $nombreArticles ?> article<?= $nombreArticles > 1 ? 's' : '' ?>)</span>
                            </h2>
                            ...
                            <?php foreach ($panier as $id => $item): ?>
                                <li class="p-6 flex flex-col sm:flex-row gap-4 cart-item" data-product-id="<?= $id ?>">
                                    ...
                                    <div class="sm:w-24 sm:h-24 flex-shrink-0 rounded-lg overflow-hidden bg-gray-100 dark:bg-gray-700">
                                        <?php if ($item['image']): ?>
                                            <img src="/public/images/produits/<?= htmlspecialchars($item['image']) ?>" alt="<?= htmlspecialchars($item['nom']) ?>" class="w-full h-full object-cover" />
                                        <?php else: ?>
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        ...
        function showToast(message, type = 'info') {
            ...
            toast.className = 'p-4 rounded-lg shadow-xl text-white text-sm'; // Tailwind classes
            if (type === 'success') {
            ...
            fetch(`${window.location.origin}/public/cart-api.php`, {
                ...
                .then(data => {
                    if (data.success) {
                        ...
                        // Mettre à jour le nombre d'articles dans le titre du panier
                        const cartTitleText = document.getElementById('cart-title-text');
                        if (cartTitleText) {
                            cartTitleText.textContent = `Votre panier (${data.cart_total_items} article${data.cart_total_items !== 1 ? 's' : ''})`;
                        }
                        // Mettre à jour un badge de panier global s'il existe
                        ...
                        showToast(`Erreur: ${data.message || 'La mise à jour de la quantité a échoué.'}`, 'error');
                    }
                })
                .catch(error => {
            ...
            fetch(`${window.location.origin}/public/cart-api.php`, {
                ...
                .then(data => {
                    if (data.success) {
                        ...
                        showToast(`Erreur: ${data.message || 'La suppression a échoué.'}`, 'error');
                    }
                })
                .catch(error => {
                    console.error('Network error:', error);
                ...
                fetch(`${window.location.origin}/public/cart-api.php`, {
                    ...
                    .then(data => {
                        if (data.success) {
                            ...
                            showToast(`Erreur: ${data.message || 'Impossible de vider le panier.'}`, 'error');
                        }
                    })
                    .catch(error => {
                        console.error('Network error:', error);
