...
<section class="bg-gray-50 dark:bg-gray-900">
    ...
    <div id="product-grid" class="container mx-auto px-4 py-8 grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
        <?php foreach ($produits as $produit) : ?>
            <div class="bg-white dark:bg-gray-800 rounded-lg shadow hover:shadow-lg transition-shadow duration-300 flex flex-col overflow-hidden">
                <div class="flex justify-center">
                    <img src="/images/<?= htmlspecialchars($produit->getImage()) ?>" alt="<?= htmlspecialchars($produit->getNom()) ?>" class="h-48 w-full object-cover" />
                </div>
                <div class="p-6 flex-grow flex flex-col">
                    <h2 class="text-lg font-semibold text-gray-900 dark:text-gray-100 mb-2"><?= htmlspecialchars($produit->getNom()) ?></h2>
                    <p class="text-gray-600 dark:text-gray-400 flex-grow"><?= nl2br(htmlspecialchars($produit->getDescription())) ?></p>
                    <br>
                    <div class="flex justify-between items-center mt-4 mb-4">
                        <div class="text-lg font-bold text-primary dark:text-primary-light"><?= htmlspecialchars($produit->getPrix()) ?> <?= htmlspecialchars($produit->getDevis()) ?></div>
                        <?php foreach ($etudiants as $etudiant) : ?>
                            <?php if ($etudiant->getId() === $produit->getEtudiantId()) : ?>
                                <div class="text-sm text-gray-500 dark:text-gray-400">Par : <?= htmlspecialchars($etudiant->getNom()) ?></div>
                            <?php endif; ?>
                        <?php endforeach; ?>
                    </div>
                    <button class="mt-auto bg-gradient-to-r from-blue-500 via-indigo-600 to-purple-700 text-white font-medium rounded-md py-2 px-4 hover:opacity-90 transition-opacity duration-200">
                        Acheter
                    </button>
                </div>
            </div>
